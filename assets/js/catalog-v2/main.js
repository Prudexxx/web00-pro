(function () {
  ...
  function isCloudPrimaryCatalog() {
    return Boolean(
      CATALOG &&
      typeof CATALOG.getConfig === "function" &&
      CATALOG.getConfig().catalogRuntimeMode === "cloud-primary"
    );
  }

  let activeSolution = isCloudPrimaryCatalog()
    ? null
    : (DATA.SOLUTIONS || []).find((item) => item.active !== false) || DATA.SOLUTIONS[0];
  let activeService = null;
  let catalogState = null;
  let lastRenderedCatalogKey = null;
  ...
  function catalogItems() {
    ...
    if (isCloudPrimaryCatalog()) return [];
    return solutions();
  }
  ...
  function solutionById(id) {
    const legacyFallback = isCloudPrimaryCatalog() ? null : DATA.SOLUTIONS[0];
    return catalogItemById(id) || catalogItems()[0] || legacyFallback;
  }
  ...
  function catalogItemById(id) {
    ...
    if (isCloudPrimaryCatalog()) return null;
  ...
  function normalizeSolutionTitle(value) {
    ...
    if (!title) return "";
    if (isCloudPrimaryCatalog()) return title;
  ...
  function canRenderCatalogState(state) {
    ...
    if (state.lifecycle !== "ready" && state.lifecycle !== "empty") return false;
    return state.freshness === "ready-current" ||
      state.freshness === "degraded-verified" ||
      state.freshness === "degraded-static";
  }

  function catalogRenderKey(state) {
    if (
      state?.source === "cloud" &&
      Number.isSafeInteger(state.revision) &&
      state.sha256
    ) {
      return `cloud:${state.revision}:${state.sha256}`;
    }

    if (state?.freshness === "degraded-static" && Array.isArray(state.items)) {
      return `static:${state.items.map((item) => item.slug || item.id || item.key || "").join("|")}`;
    }

    return null;
  }

  function skeletonCardHtml(index) {
    return `
      <article
  ...
  function shouldRetryCatalogState(state) {
    if (!state) return true;
    return state.freshness === "degraded-verified" ||
      state.freshness === "degraded-static" ||
      (!canRenderCatalogState(state) && state.freshness !== "fatal");
  }
  ...
  function applyCatalogState(nextCatalogState) {
    ...
    if (isCloudPrimaryCatalog()) {
      if (!canRenderCatalogState(nextCatalogState)) {
        updateCatalogStateNodes(nextCatalogState, {
          loading: nextCatalogState.freshness === "bootstrap" || nextCatalogState.freshness === "loading-current",
        });
        return false;
      }

      const nextKey = catalogRenderKey(nextCatalogState);
      catalogState = nextCatalogState;

      if (nextKey && nextKey === lastRenderedCatalogKey) {
        updateCatalogStateNodes(catalogState);
        return true;
      }

      if (catalogState.lifecycle === "empty") {
        clearSolutionsGrid();
      } else {
        renderSolutions();
      }

      lastRenderedCatalogKey = nextKey;
      updateCatalogStateNodes(catalogState);
      return true;
    }
  ...
  async function refreshCatalogInBackground() {
    ...
    let catalogStatusUpdated = false;
    ...
    try {
      ...
      catalogStatusUpdated = isCloudPrimaryCatalog();
      if (isCloudPrimaryCatalog()) {
        if (shouldRetryCatalogState(nextCatalogState)) scheduleCatalogRetry();
      } else if (!loaded) {
      ...
      if (!catalogStatusUpdated) updateCatalogStateNodes(catalogState);
    }
  }
  ...
  async function initCatalogState() {
    ...
    if (isCloudPrimaryCatalog()) {
      renderCatalogSkeleton(6);
      updateCatalogStateNodes(catalogState, {
        loading: catalogState?.freshness === "bootstrap" || catalogState?.freshness === "loading-current",
      });
    }
    refreshCatalogInBackground();
    return catalogState;
  ...
  function renderSolutions() {
    const grid = $("[data-solutions-grid]");
    if (!grid) return;
    if (isCloudPrimaryCatalog() && !canRenderCatalogState(catalogState)) {
      renderCatalogSkeleton(6);
      return;
    }
  ...
  async function initHome() {
    ...
    if (page === "solutions" && isCloudPrimaryCatalog()) {
      if (!canRenderCatalogState(catalogState)) renderCatalogSkeleton(6);
    } else {
      renderSolutions();
    }
    renderPopularSolutions();
