...
function readSingleFile(input, fieldName) {
...
function validateBatch(files) {
  ...
  if (files.length > IMAGE_UPLOAD_LIMITS.batchFiles) {
    throw new Error("Batch должен содержать не больше 10 файлов.");
  }
  for (const file of files) {
    validateImageFile(file);
  }
  ...
  if (total > IMAGE_UPLOAD_LIMITS.batchBytes) {
    throw new Error("Batch должен быть не больше 30 MiB.");
  }
}

function assertGalleryCapacity(gallery, selectedCount) {
